...
{
	if (Flags & FLAG_DEBUG) printf("Next Sector '%lu' is beyond end of file\n",Next);
	return(-1);
}
...
if (pos > DocLen) 
{
	if (Flags & FLAG_DEBUG) printf("Position of next sector '%llu' is beyond end of file '%llu'\n",pos,DocLen);
	return(-1);
}
...
if (DocLen < sizeof(TOLEHeader))
{
	...
	Item->ResultInfo=CommaList(Item->ResultInfo,"malformed: too short");
	return(NULL);
}
...
{
	Item->RulesResult=RULE_MALFORMED | RULE_EVIL;
	Item->ResultInfo=CommaList(Item->ResultInfo,"malformed: sector size too small");
	STREAMClose(S);
	return(NULL);
}
...
if ((Flags & FLAG_DEBUG)) printf("Check OLE: [%s]\n",Path);
...
{
	Item->RulesResult=RULE_MALFORMED | RULE_EVIL;
	Item->ResultInfo=CommaList(Item->ResultInfo,"malformed: bad sector");
}
