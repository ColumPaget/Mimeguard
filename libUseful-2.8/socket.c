...
else 
{
  ...
  if (! StrValid(Host)) sa4->sin_addr.s_addr=INADDR_ANY;
	else
	{
  if (! IsIP4Address(Host)) p_Addr=LookupHostIP(Host);
	else p_Addr=Host;
  if (StrValid(p_Addr)) sa4->sin_addr.s_addr=StrtoIP(p_Addr);
	}
